...
const find = (request, h) => {
    ...
    const filteredProducts = products.filter((product) => {
        ...
        if (attribute_type && !product.attributes.some((attributes) => attributes.type === attribute_type)) {
            return false;
        }
        if (attribute_value && !product.attributes.some((attributes) => attributes.value === attribute_value)) {
            return false;
        }
        return true;
    });
